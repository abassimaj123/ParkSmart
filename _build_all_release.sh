#!/bin/bash
# Builds release APKs for all 19 active Flutter apps in the Calcwise portfolio.
# Kotlin-native apps (TaxeCA, TaxeUK, TaxUS) are built via Android Studio / Gradle directly.
...
build_app() {
  ...
  if [ -n "$flavor" ]; then
    cd "D:/mob/$app" && flutter build apk \
      --flavor "$flavor" \
      --dart-define="FLAVOR=$dart_define" \
      -t lib/main.dart \
      --release >> "$LOG" 2>&1
  else
