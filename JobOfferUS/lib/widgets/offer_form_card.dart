...
class OfferFormCard extends StatefulWidget {
  ...
  const OfferFormCard({
    super.key, required this.isOfferA, required this.value,
    required this.isPremium, required this.isSpanish,
...
class _OfferFormCardState extends State<OfferFormCard> {
  ...
  late final TextEditingController _salary, _label, _company, _bonus,
      _match, _upTo, _health, _dental, _pto, _rsu, _commute, _raise;
  ...
  void initState() {
    ...
    _health  = _c(o.healthInsuranceSavings > 0 ? o.healthInsuranceSavings.toStringAsFixed(0) : '');
    _dental  = _c(o.dentalVisionSavings    > 0 ? o.dentalVisionSavings.toStringAsFixed(0)    : '');
    _pto     = _c(o.ptoDays > 0 ? o.ptoDays.toString() : '');
    _rsu     = _c(o.annualRsuValue     > 0 ? o.annualRsuValue.toStringAsFixed(0)     : '');
    _commute = _c(o.commuteMilesPerDay > 0 ? o.commuteMilesPerDay.toStringAsFixed(0) : '');
  ...
  @override
  void dispose() {
    for (final c in [_salary,_label,_company,_bonus,_match,_upTo,
                     _health,_dental,_pto,_rsu,_commute,_raise]) c.dispose();
    super.dispose();
  }
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(
                  ...
                  children: [
                    Text(
                      widget.value.label.isEmpty ? _offerLabel : widget.value.label,
                      style: const TextStyle(color: Colors.white,
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  // Name + Company
                  Row(children: [
                    Expanded(child: _tf(ctrl: _label,
                        label: widget.isSpanish ? 'Nombre' : 'Offer name',
                        hint: _offerLabel,
                        icon: Icons.label_outline_rounded)),
                    const SizedBox(width: 10),
                    Expanded(child: _tf(ctrl: _company,
                  ...
                  // Bonus + PTO
                  Row(children: [
                    Expanded(child: _tf(ctrl: _bonus,
                        label: widget.isSpanish ? 'Bono (%)' : 'Bonus (%)',
                        hint: '10', suffix: '%', num: true,
                        icon: Icons.card_giftcard_outlined,
                        onCh: (_) => _emit())),
                    const SizedBox(width: 10),
                    Expanded(child: _tf(ctrl: _pto,
                  ...
                  if (_showBenefits) ...[
                    const SizedBox(height: 14),
                    Row(children: [
                      Expanded(child: _tf(ctrl: _match,
                          label: '401k match (%)', hint: '100',
                          suffix: '%', num: true,
                          icon: Icons.savings_outlined,
                          onCh: (_) => _emit())),
                      const SizedBox(width: 10),
                      Expanded(child: _tf(ctrl: _upTo,
                          label: widget.isSpanish ? 'Hasta (%)' : 'Up to (%)',
                          hint: '4', suffix: '%', num: true,
                          onCh: (_) => _emit())),
                    ]),
                    const SizedBox(height: 12),
                    Row(children: [
                      Expanded(child: _tf(ctrl: _health,
                          label: widget.isSpanish ? 'Salud (\$/año)' : 'Health (\$/yr)',
                          hint: '3000', prefix: '\$', num: true,
                          icon: Icons.health_and_safety_outlined,
                          onCh: (_) => _emit())),
                      const SizedBox(width: 10),
                      Expanded(child: _tf(ctrl: _dental,
                          label: widget.isSpanish ? 'Dental (\$/año)' : 'Dental (\$/yr)',
                          hint: '500', prefix: '\$', num: true,
                          onCh: (_) => _emit())),
                    ]),
                    const SizedBox(height: 12),
                    _tf(ctrl: _rsu,
                        label: widget.isSpanish
                            ? 'RSU/Stock anual (\$)' : 'Annual RSU/Stock (\$)',
                        hint: '20000', prefix: '\$', num: true,
                    ...
                    if (!widget.value.isRemote) ...[
                      ...
                      _tf(ctrl: _commute,
                          label: widget.isSpanish
                              ? 'Km ida al trabajo' : 'Miles one-way commute',
                          hint: '15', suffix: ' mi', num: true,
                    ...
                    _tf(ctrl: _raise,
                        label: widget.isSpanish
                            ? 'Aumento anual (%)' : 'Annual raise (%)',
                        hint: '3', suffix: '%', num: true,
  ...
  Widget _salaryField() => TextField(
    ...
    style: TextStyle(
      ...
      color: _c1, letterSpacing: -0.5,
    ),
    decoration: InputDecoration(
      labelText: widget.isSpanish ? 'Salario anual (\$)' : 'Annual salary (\$)',
      hintText: '85,000',
      prefixText: '\$  ',
      prefixStyle: TextStyle(
        fontSize: 18, fontWeight: FontWeight.w700,
        color: _c1.withOpacity(0.6),
      ),
      labelStyle: TextStyle(color: _c1.withOpacity(0.7), fontWeight: FontWeight.w500),
      floatingLabelStyle: TextStyle(color: _c1, fontWeight: FontWeight.w600),
      border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(14),
        borderSide: BorderSide(color: _c1.withOpacity(0.25)),
      ),
      enabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(14),
        borderSide: BorderSide(color: _c1.withOpacity(0.22)),
      ),
      focusedBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(14),
        borderSide: BorderSide(color: _c1, width: 2),
      ),
      fillColor: _c1.withOpacity(0.08),
      filled: true,
  ...
  Widget _tf({
    ...
    String? hint, String? prefix, String? suffix, IconData? icon,
    bool num = false,
    ValueChanged<String>? onCh,
      ...
      TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixText: prefix, suffixText: suffix,
  ...
  Widget _stateDropdown() => DropdownButtonFormField<String>(
    ...
    decoration: InputDecoration(
      ...
      floatingLabelStyle: TextStyle(color: _c1, fontWeight: FontWeight.w600),
      prefixIcon: const Icon(Icons.location_on_outlined, size: 17),
    ),
  ...
  Widget _cityDropdown() {
    ...
    final cur = cities.contains(widget.value.city)
        ? widget.value.city : 'National Average';
    return DropdownButtonFormField<String>(
      initialValue: cur, isExpanded: true,
      decoration: InputDecoration(
        labelText: widget.isSpanish
            ? 'Ciudad (costo de vida)' : 'City (cost of living)',
        floatingLabelStyle: TextStyle(color: _c1, fontWeight: FontWeight.w600),
        prefixIcon: const Icon(Icons.location_city_outlined, size: 17),
  ...
  Widget _remoteToggle() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: active
            ? ct.successGreen.withOpacity(0.1)
            : ct.surfaceHigh,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: active
              ? ct.successGreen.withOpacity(0.35)
              : ct.cardBorder,
        ),
      ),
      child: Row(children: [
        Icon(
          active ? Icons.home_work_outlined : Icons.directions_car_outlined,
          size: 18,
...
class _BenefitsToggle extends StatelessWidget {
  ...
  const _BenefitsToggle({
    required this.expanded, required this.isSp,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Text(
            isSp
                ? (expanded ? 'Ocultar beneficios' : '+ Beneficios y transporte')
